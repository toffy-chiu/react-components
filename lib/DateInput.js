...
var DateInput = React.createClass({
    ...
    _setValue: function _setValue(value) {
        if (typeof this.props.onChange === 'function') {
            ...
            this.props.onChange({ target: element });
        } else {
    ...
    render: function render() {
        return React.createElement('input', { ref: 'ele', name: this.props.name, value: this.props.value, type: 'text', onClick: this.show, style: { cursor: 'pointer' }, readOnly: true });
    }
});
    ...
    function findChildren(children) {
        if (children.length) {
            for (var i = 0, len = children.length; i < len; i++) {
                ...
                if (children[i] === childNode || findChildren(children[i].childNodes)) {
                    return true;
                }
                //否则继续寻找
            }
...
function moveMonth(date, dir) {
    ...
    if (mag == 1) {
        ...
        for (var i = 0; i < mag; i++) {
            ...
            new_date = moveMonth(new_date, dir);
        }
